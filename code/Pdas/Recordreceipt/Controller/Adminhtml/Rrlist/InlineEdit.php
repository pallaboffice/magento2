        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Pdas\Recordreceipt\Model\Rrlist $model */
                    $model = $this->_objectManager->create('Pdas\Recordreceipt\Model\Rrlist')->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Rrlist ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
